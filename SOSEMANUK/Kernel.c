	...
	__kernel void Make_stream(__global unsigned int* lfsr, __global unsigned int* fsmR, __global unsigned int* ILOSC_INTOW_NA_RAZ, __global unsigned int* ILOSC_WEKTOROW, __global unsigned int* output_stream, __global unsigned int* output_lsfr, __global unsigned int* output_fsmR)
	{
	
		unsigned int mulAlpha[] = {
		...
		//output_stream[0] = n;
	// To potrzeba dlatego ¿e nie wszystkie wrokitemy powinny wykonywaæ na raz obliczenia bo mo¿e braknac pamiêci
		if (n >= *ILOSC_WEKTOROW)
			return;


		int seria = 0;
		for (seria = 0; seria < *ILOSC_INTOW_NA_RAZ/4; seria++)
		{

		// LFSR & FSM
			unsigned int fsm_klucz[4];
			unsigned int lfsr_klucz[4];
			int i = 0;
			...
			for (i = 0; i < 4; i++)
			{
				...
				// FSM
				unsigned int x = fsmR[2 * n + 0];
				fsmR[2 * n + 0] = fsmR[2 * n + 1] + (lfsr[10 * n + 1] ^ ((fsmR[2 * n + 0] & 0x01) != 0 ? lfsr[10 * n + 8] : 0));
				fsmR[2 * n + 1] = Rol(x * 0x54655307, 7);

				fsm_klucz[i] = (lfsr[10 * n + 9] + fsmR[2 * n + 0]) ^ fsmR[2 * n + 1];

				// LFSR
				unsigned int v1 = lfsr[10 * n + 9];
				unsigned int v2 = (lfsr[10 * n + 3] >> 8) ^ divAlpha[lfsr[10 * n + 3] & 0xFF];
				unsigned int v3 = (lfsr[10 * n + 0] << 8) ^ mulAlpha[lfsr[10 * n + 0] >> 24];
				lfsr_klucz[i] = lfsr[10 * n + 0];

				int j = 0;
				for (j = 0; j < 9; j++) {
					lfsr[10 * n + j] = lfsr[10 * n + j + 1];
				}

				lfsr[10 * n + 9] = v1 ^ v2 ^ v3;
				//
			}
			...
			int j = 0;
			for (j = 0; j < 4; j++) {
				output_stream[seria * (4 * *ILOSC_WEKTOROW) + 4 * n + j] = LittleEndian(fsm_klucz[3 - j] ^ lfsr_klucz[j]);
			}	
		}

		output_fsmR[n + 0] = fsmR[0];
		output_fsmR[n + 1] = fsmR[1];

		int i = 0;
		for (i = 0;i < 10; i++) {
			output_lsfr[10 * n + i] = lfsr[i];
		}
	...
	__kernel void Key_schedule_IV(__global unsigned int* klucz, __global unsigned int* IV, __global unsigned int* output, __global unsigned int* ILOSC_WEKTOROW)
	{

		unsigned int w[108] = { 0 };
	    unsigned int subkey[100] = { 0 };
		unsigned int n = get_global_id(0);

		if (n > ILOSC_WEKTOROW - 1)
			return;
